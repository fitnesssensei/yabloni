...
- Для продакшена обязательно сгенерируйте новый 
безопасный ключ и замените его в .env. 
Не используйте значение, начинающееся с django-insecure-.

- Включён режим DEBUG: DEBUG = True в продакшене раскрывает
 детальную информацию об ошибках, включая системную 
 информацию, запросы к базе данных и переменные окружения.

- Пустой ALLOWED_HOSTS: ALLOWED_HOSTS = [] позволяет 
потенциальные атаки через заголовок хоста. Должен 
быть настроен со специфическими разрешёнными доменами.

- Логирование чувствительных данных: В представлениях 
есть операторы print, которые выводят пароли электронной 
почты и данные заказов в консольные логи, которые могут 
быть доступны в продакшене.
